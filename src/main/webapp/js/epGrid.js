...
(function ($)
{
    ...
    $.fn.epGrid = function (options)
    {
		return $.epui.run.call(this, "epGrid", arguments);
    };

    $.fn.epGetGridManager = function ()
    {
        return $.epui.run.call(this, "epGetGridManager", arguments);
    };
    ...
    $.epDefaults.Grid = {
        ...
        switchPageSizeApplyComboBox: false,     //切换每页记录数是否应用epComboBox ？
        ...
        toolbar: null,                           //工具条,参数同 epToolbar的
    ...
    $.epDefaults.GridString = {
        errorMessage: uncompile(epLanguage.epGrid.errorMessage),
        pageStatMessage:uncompile(epLanguage.epGrid.pageStatMessage),
        pageTextMessage: uncompile(epLanguage.epGrid.pageTextMessage),
        loadingMessage: uncompile(epLanguage.epGrid.loadingMessage),
        findTextMessage: uncompile(epLanguage.epGrid.findTextMessage),
        noRecordMessage: uncompile(epLanguage.epGrid.noRecordMessage),
        isContinueByDataChanged: uncompile(epLanguage.epGrid.isContinueByDataChanged),
        cancelMessage: uncompile(epLanguage.epGrid.cancelMessage),
        saveMessage: uncompile(epLanguage.epGrid.saveMessage),
        applyMessage: uncompile(epLanguage.epGrid.applyMessage),
        draggingMessage: uncompile(epLanguage.epGrid.draggingMessage)
    };
    //接口方法扩展
    $.epMethos.Grid = $.epMethos.Grid || {};

    //排序器扩展
    $.epDefaults.Grid.sorters = $.epDefaults.Grid.sorters || {};

    //格式化器扩展
    $.epDefaults.Grid.formatters = $.epDefaults.Grid.formatters || {};

    //编辑器扩展
    $.epDefaults.Grid.editors = $.epDefaults.Grid.editors || {};


    //排序
	$.epDefaults.Grid.sorters['date'] = function (val1, val2)
    {
        return val1 < val2 ? -1 : val1 > val2 ? 1 : 0;
    };
    $.epDefaults.Grid.sorters['int'] = function (val1, val2)
    {
        return parseInt(val1) < parseInt(val2) ? -1 : parseInt(val1) > parseInt(val2) ? 1 : 0;
    };
    $.epDefaults.Grid.sorters['float'] = function (val1, val2)
    {
        return parseFloat(val1) < parseFloat(val2) ? -1 : parseFloat(val1) > parseFloat(val2) ? 1 : 0;
    };
    $.epDefaults.Grid.sorters['string'] = function (val1, val2)
    {
    ...
    //日期格式化
	$.epDefaults.Grid.formatters['date'] = function (value, column)
    {
...
//文本框编辑器
     $.epDefaults.Grid.editors['text'] = {
         create: function (container, editParm)
         {
			if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
				...
				if(column.editor.inputMode){
				 ...
				 if(column.editor.inputLinkHandler){
					 $(input).on('input',function(e){
						 try{
							 var fun = column.editor.inputLinkHandler;
							 if ((typeof fun) == "function") {
									fun.apply(null,[e,editParm]);
								}
							 //inputLinkHandler(e,editParm);
						 }
						 catch(e){
	 ...
	 //文本域编辑器
     $.epDefaults.Grid.editors['textarea'] = {
         create: function (container, editParm)
         {
            if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
				var column = editParm.column;
	...
	//下拉框
    $.epDefaults.Grid.editors['select'] =
     {
         create: function (container, editParm,g,p)
         {
             if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
			 ...
			 input.attr("prompt",uncompile(epLanguage.epGrid.editorPromptMessage));
			 if(column.editor.detailEdit==true){
				container.width('auto').height(30);
			}
			else{
	 ...
	 //树形下拉框编辑器
    $.epDefaults.Grid.editors['selectTree'] =
     {
         create: function (container, editParm,g,p)
         {
             if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
	  ...
	  //组合下拉框编辑器
    $.epDefaults.Grid.editors['selectCustom'] =
     {
	 ...
	 //自动提示框编辑器
    $.epDefaults.Grid.editors['suggestion'] =
     {
         create: function (container, editParm,g,p)
         {
			 if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
    ...
    //步进器编辑器
     $.epDefaults.Grid.editors['stepper'] =
     {
        create: function (container, editParm)
         {
             if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
...
$.epDefaults.Grid.editors['radio'] =
        ...
        create: function (container, editParm)
         {
             if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))=="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){
				...
				//alert(editParm)
				var inputStr = "<span>"
				var dataList = column.editor.data.list;
				for(var i=0;i<dataList.length;i++){
					//alert(dataList[i].value)
					inputStr = inputStr + "<input type='radio' value='"+dataList[i].value+"' id='radio1"+editParm.rowindex+"' name='radio"+editParm.rowindex+"'/>";
					inputStr = inputStr + "<label for='radio1"+editParm.rowindex+"' class='hand'>"+dataList[i].key+"</label>";
				}
				inputStr = inputStr + "</span>";
			 var input = $(inputStr);
			 if(column.editor.detailEdit==true){
					container.width('auto').height(30);
				}
         ...
         getValue: function (input, editParm)
         {
         	//alert(input.find("input").filter("[checked]").val());
             return input.find("input").filter("[checked]").val();//input.val();
         },
         setValue: function (input, value, editParm)
         {
    ...
    $.epui.controls.Grid = function (element, options)
    {
        $.epui.controls.Grid.base.constructor.call(this, element, options);
    };

    //扩展
	$.epui.controls.Grid.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return '$.epui.controls.Grid';
        },
        __idPrev: function ()
        {
            return 'grid';
        ...
        {
            return $.epMethos.Grid;
        },
        _init: function ()
        {
            $.epui.controls.Grid.base._init.call(this);
            var g = this, p = this.options;
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnfirst' title='首页'><span></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnprev' title='前一页'><span></span></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            ...
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnnext' title='下一页'><span></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnlast' title='尾页'><span></span></div>");
            gridhtmlarr.push("                </div>");
            gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnload' title='刷新'><span></span></div>");
            gridhtmlarr.push("                </div>");
			if(p.showPageInfo){
				gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			}
        ...
        loadData: function (loadDataParm,detailRemory)
        {
            ...
            var clause = null;
            var loadServer = true;
			if(getTypeFlage()!="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){return;}
            ...
            else
            {
                ...
                //g.loadServerData.epDefer(g, 10, [param, clause]);
            }
			//保持上次打开的子表
		...
		loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    g._showData.epDefer(g, 10, [g.currentData]);
                },
                complete: function ()
                {
                    ...
                    {
                        g.toggleLoading.epDefer(g, 10, [false]);
                    }
        ...
        deleteSelectedRow: function ()
        {
            ...
            for (var i in this.selected)
            {
                ...
                if (o['__id'] in this.records)
                    this._deleteData.epDefer(this, 10, [o]);
            }
            this.reRender.epDefer(this, 20);
        },
        ...
        */
		...
		updateRow: function (rowDom, newRowData)
        {
            ...
            if (rowdata[p.statusName] != 'add')
                rowdata[p.statusName] = 'update';
            g.reRender.epDefer(g, 10, [{ rowdata: rowdata}]);
            return rowdata;
        },
        //编辑单元格
        ...
        toggleCol: function (columnparm, visible, toggleByPopup)
        {
            ...
            if (!toggleByPopup)
            {
                $(':checkbox[columnindex=' + columnindex + "]", g.popup).each(function ()
                {
                    ...
                    if ($.fn.epCheckBox)
                    {
                        var checkboxmanager = $(this).epGetCheckBoxManager();
       ...
       //得到子表
        ...
        _initBuildHeader: function ()
        {
            ...
            {
                if ($.fn.epToolBar)
                    g.toolbarManager = g.topbar.epToolBar(p.toolbar);
            }
            else
            {
                g.topbar.remove();
        ...
        _createHeaderCell: function (column)
        {
			...
			//改变列宽
            if (!column.issystem && column['__leaf'] && column.resizable !== false && $.fn.epResizable)
            {
               //使用epResizable
			    g.colResizable[column['__id']] = jcell.epResizable({ handles: 'e',
        ...
        _initBuildPopup: function ()
        {
            ...
            if ($.fn.epCheckBox)
            {
                $('input:checkbox', g.popup).epCheckBox(
                {
       ...
       //分页处添加转跳的下拉框
	    _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
				...
				$('.l-bar-selectpagesize', g.toolbar).append("<span>每页</span><select name='rp'>" + optStr + "</select><span>条</span>");
                if (selectedIndex != -1) $('.l-bar-selectpagesize select', g.toolbar)[0].selectedIndex = selectedIndex;
                if (p.switchPageSizeApplyComboBox && $.fn.epComboBox)
                {
                    $(".l-bar-selectpagesize select", g.toolbar).epComboBox(
                    {
        ...
        {
            var g = this, p = this.options;
				if((eval(uncompile("u%CC%D9%C8%CD%E9%D5%AB%B2%CD%C8%CC%8DQ")))!="r%C8%D0%D9%D3%CB%C3%CF%D0%C7%D3"){return;}
				//加载数据 
		...
		_getHtmlFromData: function (data, frozen)
        {
            ...
            if(data.length==0&&!frozen){//如果记录数为0，提示没有记录
            	return '<tr><td style="width:400px;height:26px;min-height:26px;" >'+p.noRecordMessage+'</td></tr> ';
            }else{
            	return gridhtmlarr.join('');
            }
        },
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            //张京涛
//            if(column.hyperlink){
//            	content="<a href='javascript:void(0);' class='hyperlink'>"+value+"</a>";
...
//            }
            if(column.sysCode){
            	if(column._sysCodeListData){
            		
            	}
            	else{
            		$.ajax({ 
        				url: jsContextPath+"/upp/syscode/querySysCodeForSelect.do", 
        				dataType:"json",
        				data:column.sysCode,
        				async:false,
        				error:function(){
        				...
        				success: function(data){
        					column._sysCodeListData = data;
        					
        			    }
        				});
            	}
            	for(index in column._sysCodeListData.list){
					if(value==column._sysCodeListData.list[index].value){
						content = column._sysCodeListData.list[index].key;
						value=column._sysCodeListData.list[index].key;
					}
        ...
        _setEvent: function ()
        {
            ...
            if ($.fn.epDrag && p.colDraggable)
            {
               ...
               //使用epDrag
			    g.gridheader.add(g.f.gridheader).epDrag({ revert: true, animate: false,
                    proxyX: 0, proxyY: 0,
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        if (g.colDropIn != -1)
                        {
                            g.changeCol.epDefer(g, 0, [column, g.colDropIn, g.colDropDir == "right"]);
                            g.colDropIn = -1;
                        }
                        g.colDroptip.hide();
                        this.set('cursor', 'default');
            ...
            //行拖拽支持：交换行的位置
            if ($.fn.epDrag && p.rowDraggable)
            {
                g.rowDroptip = $("<div class='l-drag-rowdroptip' style='display:none'></div>").appendTo('body');
                g.gridbody.add(g.f.gridbody).epDrag({ revert: true, animate: false,
                    proxyX: 0, proxyY: 0,
                    proxy: function (draggable, e)
                    {
                        var src = g._getSrcElementByEvent(e);
        ...
        _onKeyDown: function (e)
        {
			...
			if(p.excelMode){
	            ...
	            var keyCode = e.keyCode || e.which || e.charCode;		
				 if (g.editor.editing && !$.epui.win.masking) {
					if (keyCode == 37) {//左
        ...
        _onClick: function (e)
        {
			...
			if (src.out)
            {
                if (g.editor.editing && !$.epui.win.masking) g.endEdit();
...
*/
            ...
            else if (src.hcelltext) //排序
            {
                ...
                //zjt
                if(p.toFirstPageAfterSort==true||p.toFirstPageAfterSort=="true"){
                	p.newPage = 1
                }
            ...
            {
                if (!src.checkbox){
				 	g.trigger('clickRow', [ src.data, src.data['__id'], src.row,e]);
				 }
				//复选框
    ...
    $.epui.controls.Grid.prototype.enabledTotal = $.epui.controls.Grid.prototype.isTotalSummary;
    $.epui.controls.Grid.prototype.add = $.epui.controls.Grid.prototype.addRow;
    $.epui.controls.Grid.prototype.update = $.epui.controls.Grid.prototype.updateRow;
    $.epui.controls.Grid.prototype.append = $.epui.controls.Grid.prototype.appendRow;
    $.epui.controls.Grid.prototype.getSelected = $.epui.controls.Grid.prototype.getSelectedRow;
    $.epui.controls.Grid.prototype.getSelecteds = $.epui.controls.Grid.prototype.getSelectedRows;
    $.epui.controls.Grid.prototype.getCheckedRows = $.epui.controls.Grid.prototype.getSelectedRows;
    $.epui.controls.Grid.prototype.getCheckedRowObjs = $.epui.controls.Grid.prototype.getSelectedRowObjs;
    $.epui.controls.Grid.prototype.setOptions = $.epui.controls.Grid.prototype.set;
...
(function ($)
{
    $.fn.epResizable = function (options)
    {
        return $.epui.run.call(this, "epResizable", arguments,
        {
            idAttrName: 'epuiresizableid', hasElement: false, propertyToElemnt: 'target'
        });
    };

    $.fn.epGetResizableManager = function ()
    {
        return $.epui.run.call(this, "epGetResizableManager", arguments,
        {
            idAttrName: 'epuiresizableid', hasElement: false, propertyToElemnt: 'target'
        });
    };
    ...
    $.epui.controls.Resizable = function (options)
    {
        $.epui.controls.Resizable.base.constructor.call(this, null, options);
    };

    $.epui.controls.Resizable.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return 'Resizable';
        },
        ...
        _render: function ()
        {
            ...
            g.target.mousemove(function (e)
            {
                ...
                if (p.target.epuidragid)
                {
                    var drag = $.epui.get(p.target.epuidragid);
                    if (drag && g.dir)
                    {
        ...
        _rendered: function ()
        {
            this.options.target.epuiresizableid = this.id;
        },
        _getDir: function (e)
        {
            var g = this, p = this.options;
...
(function ($)
{
    ...
    $.fn.epToolBar = function (options)
    {
        return $.epui.run.call(this, "epToolBar", arguments);
    };

    $.fn.epGetToolBarManager = function ()
    {
        return $.epui.run.call(this, "epGetToolBarManager", arguments);
    };
    ...
    $.epui.controls.ToolBar = function (element, options)
    {
        $.epui.controls.ToolBar.base.constructor.call(this, element, options);
    };
    $.epui.controls.ToolBar.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return 'ToolBar';
        },
