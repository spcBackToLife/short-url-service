...
$(function(){
	...
	if (browserMatch.browser&&getTypeFlag2()) {
	...
	if($("#skin").attr("splitMode")==true||$("#skin").attr("splitMode")=="true"){
		...
		try {
			...
			if($("body").attr("leftFrame")!="true"){
				alert(uncompile(epLanguage.jsError.splitModeMessage));
			}
			return;
		}
		...
		if(autoGetSkin==true&&splitMode==false){
			if($parentThemeDom.attr("debug")=="true"||$parentThemeDom.attr("debug")==true){
				if(broswerFlag=="IE6"||broswerFlag=="IE7"){
					if($parentThemeDom.attr("href")==""){//当子页面单独打开时
						...
						//检测是否正确设置了prePath
						$.ajax({
							url: prePath+"epui/skins/"+themeColor+"/style.css",
							error:function(){
								if($("body").attr("leftFrame")=="true"||$("body").attr("leftFrame")==true){
									alert(uncompile(epLanguage.jsError.pathMessage0)+prePath+"epui/skins/"+themeColor+"/style.css"+uncompile(epLanguage.jsError.pathMessage2));
								}
								else{
									alert(uncompile(epLanguage.jsError.pathMessage1)+prePath+"epui/skins/"+themeColor+"/style.css"+uncompile(epLanguage.jsError.pathMessage2));
								}
							},
							success:function(){
								if($("body").attr("leftFrame")=="true"||$("body").attr("leftFrame")==true){
									alert(uncompile(epLanguage.jsError.pathMessage3));
								}
								else{
									alert(uncompile(epLanguage.jsError.pathMessage4));
								};
								$.ajax({
									url: prePath+skinName+"style.css",
									error:function(){
										if($("body").attr("leftFrame")=="true"||$("body").attr("leftFrame")==true){
											alert(uncompile(epLanguage.jsError.pathMessage0)+prePath+skinName+"style.css"+uncompile(epLanguage.jsError.pathMessage5));
										}
										else{
											alert(uncompile(epLanguage.jsError.pathMessage1)+prePath+skinName+"style.css"+uncompile(epLanguage.jsError.pathMessage5));
										}
									},
									success:function(){
										if($("body").attr("leftFrame")=="true"||$("body").attr("leftFrame")==true){
											alert(uncompile(epLanguage.jsError.pathMessage6));
										}
										else{
											alert(uncompile(epLanguage.jsError.pathMessage6));
										}
			...
			$("#skin").attr("href",prePath+"epui/skins/"+themeColor+"/style.css");
	...
	if (autoRender == true) {
		...
		$(".spliter").each(function(){
			try{
				...
				alert(uncompile(epLanguage.jsError.spliter))
			}
		})
	}
...
(function($) {
			...
			if($(this).attr("type")=="text"){
				...
				if($(this).hasClass("color")){
					...
					try{
						...
						alert(uncompile(epLanguage.jsError.color))
					}
				}
				//日期选择框
				...
				//软键盘控件
					...
					try{
						...
						alert(uncompile(epLanguage.jsError.keypad))
					}
				}
				//数字步进器
					...
					try{
						...
						alert(uncompile(epLanguage.jsError.stepper))
					}
				}
				//文本框
				else{
		...
		//table
			...
			if($(this).hasClass("tableStyle")){
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.treeTable))
				}
			}
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.detailTable))
				}
			}
		}
		//a
			...
			if($(this).hasClass("imgPreview")){
				try{
					...
					alert(uncompile(epLanguage.jsError.imgPreview))
				}
			}
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.imgZoom))
				}
			}
		}
		//img
			...
			if($(this).hasClass("imgFrame")){
				try{
					...
					alert(uncompile(epLanguage.jsError.imgFrame))
				}
			}
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.imgFade))
				}
			}
		}
		else if($(this).is("div")){
			...
			//浮动面板
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.floatPanel))
				}
			}
			//树形下拉框
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.selectTree))
				}
			}
			//自定义下拉框
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.selectCustom))
				}
			}
			//自动提示框
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.suggestion))
				}
			}
			//条件过滤器
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.filter))
				}
			}
			//双向选择器
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.lister))
				}
			}
			//树形双选器
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.listerTree))
				}
			}
			//评星级控件
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.rating))
				}
			}
			//弹出式菜单
			...
			//基本选项卡
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.basicTab))
				}
			}
				...
				//}
				//catch(e){
					//alert(uncompile(epLanguage.jsError.basicTabModern)
				//}
			}
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.basicTabProcess))
				}
			}
			//纵向选项卡
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.verticalTab))
				}
			}
			//单级纵向导航
			...
			//抽屉容器
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.accordion))
				}
			}
			//图标导航 鼠标移入变色
			...
			//分页组件
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.pageNumber))
				}
			}
				...
				try{
					...
					alert(uncompile(epLanguage.jsError.pageArrow))
				}
			}
		}
	...
	$.fn.box2Build= function(){
		...
		var useStatusText=uncompile(epLanguage.box2.collapseText);
		if ($(this).attr("statusText") != null) {
			useStatusText=$(this).attr("statusText");
		}
		var afterStatusText=uncompile(epLanguage.box2.expendText);
	...
	$.fn.fileRender = function() {
		...
		$(this).change(function() {
			...
			else if(broswerFlag=="IE9"){
				filePath = $(this).val().toString();
			}
		...
		$(this).bind("keydown", function (e)
        {
			...
			if($(this).attr("checkCaps")!=="false"){
				$(this).caps(function(caps){
				    ...
				    if(caps){
						$(this).tip({showCloseBtn:true,content: uncompile(epLanguage.jsError.capslock),width:160});
				    }else{
				    }
				});
			}
	...
	$.fn.dateRender = function() {
		...
		$(this).focus(function(){
			try{
				...
				alert(uncompile(epLanguage.jsError.WdatePicker))
			}
				...
				if ($(this).attr("useMultColor") == "true") {
					if ($(this).attr("useCheckBox") == "false") {//如果checkBox为false采用普通多选模式
						...
						if($(this).find("th").length>0){
							var $checkIcon=$('<img src='+prePath+'epui/icons/checkAllOff.gif title="'+uncompile(epLanguage.table.selectAllBtnText)+'" class="hand"></span>');
							...
							if ($(this).attr("headFixMode") == "true"){
								$checkIcon.toggle(function(){
									...
									$(this).attr("src",prePath+'epui/icons/checkAllOn.gif');
									$(this).attr("title",uncompile(epLanguage.table.deSelectAllBtnText));
								},function(){
									$("table:[class=tableStyle]").find("tr").each(function(){
									...
									$(this).attr("src",prePath+'epui/icons/checkAllOff.gif');
									$(this).attr("title",uncompile(epLanguage.table.selectAllBtnText));
								});
							}else{
								$checkIcon.toggle(function(){
...
jQuery.SelectBox = function(selectobj) {
	...
	$loader.text(uncompile(epLanguage.select.loadingMessage));
	...
	function getSelectOptions(parentid) {
		...
		if(option.length==0){
			...
			if(!($select.attr("url"))){
				if($select.attr("params")){
					ajaxMode=true;
					$select.attr("url",jsContextPath+"/upp/syscode/querySysCodeForSelect.do");
				}
		...
		if(ajaxMode==true){
			...
			if(paramsStr){
				...
				try {
					...
					paramsObj="";
					alert(uncompile(epLanguage.select.paramErrorMessage))
				}
			}
			else{
				paramsObj="";
			...
			if(dataObj3){
				...
				try {
					...
					dataObj2="";
					alert(uncompile(epLanguage.select.dataErrorMessage))
				}
				...
				data:paramsObj,
				error:function(){
					alert(uncompile(epLanguage.select.urlErrorMessage))
				},
				success: function(xml){
	...
	function createOptions_xml(data,parentid,colNum,colWidth,isEditable,rel,ul,dataRoot,paramsObj){
					...
					//获取无关项文字内容
					var promptText=uncompile(epLanguage.select.promptMessage);
					if($select.attr("prompt")!=null){
						if($select.attr("prompt")==""){
							promptText=uncompile(epLanguage.select.promptMessage);
						}
						else{
	...
	function loadLater(obj,value){
					...
					if (obj.attr("childDataType") == "xml") {
						...
						if($(value).find(dataRoot).length==0){
							var li = document.createElement('li');
							$(li).text(uncompile(epLanguage.select.ldError));
							$childUL.append($(li));
						}
						if(selectedIdx==-1&&selectedValue==""){
						...
						$.getJSON(dataPath,function(data){
							...
							if(data.length==0){
								var li_j = document.createElement('li');
								$(li_j).text(uncompile(epLanguage.select.noItemMessage));
								$childUL_j.append($(li_j));
							}
...
(function($) 
{
	$.fn.maxlength = function(options)
	{
		var settings = jQuery.extend(
		{
			...
			status:				      true, 
			statusClass:		    "maxNum", 
			statusText:			    uncompile(epLanguage.maxlength.statusText),
			notificationClass:	"notification",	
			showAlert: 			    false,
			alertText:			   uncompile(epLanguage.maxlength.alertText), 
			slider:				      true
		}, options );
...
$.fn.password_strength = function(options)
{
	var settings = $.extend({
		...
		'minLength' : 6,
		'texts' : {
			1 : uncompile(epLanguage.passStrength.text1),
			2 : uncompile(epLanguage.passStrength.text2),
			3 : uncompile(epLanguage.passStrength.text3),
			4 : uncompile(epLanguage.passStrength.text4),
			5 : uncompile(epLanguage.passStrength.text5)
		}
	}, options);
...
function showProgressBar(str,type){
	var titleStr=uncompile(epLanguage.progressBar.title);
	if(str){
		titleStr=str;
	}
	...
	$instance.find("li a").each(function(i){
		...
		$(this).click(function(){
			...
			if($(this).attr("href")!=null){
				...
				if(boxType=="single"){
					if(data["title"]){
						top.positionContent="【"+uncompile(epLanguage.position.title)+data["title"]+">>"+$(this).text()+"】";
					}
					else{
						top.positionContent="【"+uncompile(epLanguage.position.title)+$(this).text()+"】";
					}
				}
				else{
					if(data["title"]){
						top.positionContent="【"+uncompile(epLanguage.position.title)+data["title"]+">>"+$(this).parents("ul").prev(".subtitle").eq(0).text()+">>"+$(this).text()+"】";
					}
					else{
						top.positionContent="【"+uncompile(epLanguage.position.title)+$(this).parents("ul").prev(".subtitle").eq(0).text()+">>"+$(this).text()+"】";
					}
				}
...
{
    //epui 继承方法
    Function.prototype.epExtend = function (parent, overrides)
    {
    ...
    //延时加载
    Function.prototype.epDefer = function (o, defer, args)
    {
        var fn = this;
    ...
    window.ep = $.epui = {
        ...
        //错误提示
        error: {
            managerIsExist: uncompile(epLanguage.jsError.idInfo)
        },
        getId: function (prev)
        {
        ...
        remove: function (arg)
        {
            ...
            {
                delete $.epui.managers[arg];
            }
            else if (typeof arg == "object" && arg instanceof $.epui.core.Component)
            {
                delete $.epui.managers[arg.id];
            }
        },
        //获取epui对象
        //1,传入epui ID
        ...
        {
            idAttrName = idAttrName || "epuiid";
            if (typeof arg == "string" || typeof arg == "number")
            {
                return $.epui.managers[arg];
            }
            else if (typeof arg == "object" && arg.length)
            {
                if (!arg[0][idAttrName] && !$(arg[0]).attr(idAttrName)) return null;
                return $.epui.managers[arg[0][idAttrName] || $(arg[0]).attr(idAttrName)];
            }
            return null;
        },
        //根据类型查找某一个对象
        ...
        //$.fn.ep{Plugin} 和 $.fn.epGet{Plugin}Manager
        //会调用这个方法,并传入作用域(this)
        //@parm [plugin]  插件名
        //@parm [args] 参数(数组)
        ...
        run: function (plugin, args, ext)
        {
            if (!plugin) return;
            ext = $.extend({
                defaultsNamespace: 'epDefaults',
                methodsNamespace: 'epMethods',
            ...
            plugin = plugin.replace(/^epGet/, '');
            plugin = plugin.replace(/^ep/, '');
            if (this == null || this == window || ext.isStatic)
            {
                if (!$.epui.plugins[plugin])
                {
                    $.epui.plugins[plugin] = {
                        fn: $['ep' + plugin],
                        isStatic: true
                ...
                return new $.epui[ext.controlNamespace][plugin]($.extend({}, $[ext.defaultsNamespace][plugin] || {}, $[ext.defaultsNamespace][plugin + 'String'] || {}, args.length > 0 ? args[0] : {}));
            }
            if (!$.epui.plugins[plugin])
            {
                $.epui.plugins[plugin] = {
            ...
            if (/Manager$/.test(plugin)) return $.epui.get(this, ext.idAttrName);
            this.each(function ()
            {
                if (this[ext.idAttrName] || $(this).attr(ext.idAttrName))
                {
                    var manager = $.epui.get(this[ext.idAttrName] || $(this).attr(ext.idAttrName));
                ...
                if (ext.hasElement)
                {
                    new $.epui[ext.controlNamespace][plugin](this, p);
                }
                else
                {
                    new $.epui[ext.controlNamespace][plugin](p);
                }
            });
            if (this.length == 0) return null;
            if (args.length == 0) return $.epui.get(this, ext.idAttrName);
            if (typeof args[0] == 'object') return $.epui.get(this, ext.idAttrName);
            if (typeof args[0] == 'string')
            {
                var manager = $.epui.get(this, ext.idAttrName);
    ...
    //关联起来
    $.epui.defaults = $.epDefaults;
    $.epui.methods = $.epMethos;

    //获取epui对象
    //@parm [plugin]  插件名,可为空
    $.fn.ep = function (plugin)
    {
        if (plugin)
        {
            return $.epui.run.call(this, plugin, arguments);
        }
        else
        {
            return $.epui.get(this);
        }
    };
    ...
    $.epui.core.Component = function (options)
    {
        //事件容器
    ...
    $.extend($.epui.core.Component.prototype, {
        __getType: function ()
        {
            return '$.epui.core.Component';
        },
        __idPrev: function ()
        {
            return 'epui';
    ...
    //@parm [options] 组件的参数
    $.epui.core.UIComponent = function (element, options)
    {
        $.epui.core.UIComponent.base.constructor.call(this, options);
    ...
    $.epui.core.UIComponent.epExtend($.epui.core.Component, {
        __getType: function ()
        {
            return '$.epui.core.UIComponent';
        },
        //扩展方法
        ...
        _init: function ()
        {
            ...
            if (!this.element)
            {
                this.id = this.options.id || $.epui.getId(this.__idPrev());
            }
            else
            {
                this.id = this.options.id || this.element.id || $.epui.getId(this.__idPrev());
            }
            //存入管理器池
            $.epui.add(this);
            ...
            //读取epui这个属性，并加载到参数，比如 epui = "width:120,heigth:100"
            var p = this.options;
            if ($(this.element).attr("epui"))
            {
                try
                {
                    var attroptions = $(this.element).attr("epui");
        ...
        _rendered: function ()
        {
            if (this.element)
            {
                $(this.element).attr("epuiid", this.id);
            }
        },
        //返回要转换成epui参数的属性,比如['url']
        attr: function ()
        {
            return [];
        },
    ...
    //表单控件基类
    $.epui.controls.Input = function (element, options)
    {
        $.epui.controls.Input.base.constructor.call(this, element, options);
    };

    $.epui.controls.Input.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return '$.epui.controls.Input';
        },
        attr: function ()
        {
            return ['nullText'];
    ...
    $.epui.win = {
        ...
        mask: function (win)
        {
            function setHeight()
            {
                if (!$.epui.win.windowMask) return;
                var h = $(window).height() + $(window).scrollTop();
                $.epui.win.windowMask.height(h);
            }
            if (!this.windowMask)
            {
                this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
                $(window).bind('resize.epuiwin', setHeight);
        ...
        unmask: function (win)
        {
            ...
            {
                var winid = jwins.eq(i).attr("epuiid");
                if (win && win.id == winid) continue;
                //获取epui对象
                var winmanager = $.epui.get(winid);
        ...
        {
            var wins = $.epui.find($.epui.core.Win);
            for (var i in wins)
            {
                var w = wins[i];
                if (w == win)
    ...
    //窗口基类 window、dialog
    $.epui.core.Win = function (element, options)
    {
        $.epui.core.Win.base.constructor.call(this, element, options);
    };

    $.epui.core.Win.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return '$.epui.controls.Win';
        },
        mask: function ()
        {
    ...
    $.epui.toJSON = typeof JSON === 'object' && JSON.stringify ? JSON.stringify : function (o)
    {
        var f = function (n)
        {
            return n < 10 ? '0' + n : n;
        ...
        if (type === 'object')
        {
            ...
            if (o.constructor === Array)
            {
                ...
                {
                    pairs.push($.epui.toJSON(o[i]) || 'null');
                }
                return '[' + pairs.join(',') + ']';
            }
            var name, val;
...
(function ($)
{
    ...
    $.tip = function (p)
    {
        return $.epui.run.call(null, "epTip", arguments);
    };
    ...
    $.fn.tip = function (options)
    {
        this.each(function ()
        {
            var p = $.extend({}, $.epDefaults.Tip, options || {});
			//var p= this.options;
            ...
            if (p.auto || options == undefined)
            {
                ...
                }).bind('mouseout.tip', function ()
                {
                    ...
                    var tipmanager = $.epui.managers[this.epuitipid];
                    if (tipmanager)
                    {
                        tipmanager.remove();
                    }
        ...
        return $.epui.get(this, 'epuitipid');
    };
    //关闭指定在Dom Element(附加了epui对象ID,属性名"epuitipid")显示的气泡
    $.fn.hideTip = function (options)
    {
        return this.each(function ()
        {
            ...
            {
                //如果是lable，将查找指定的input，并找到epui对象ID
            ...
            var tipmanager = $.epui.managers[target.epuitipid];
            if (tipmanager)
            {
                tipmanager.remove();
            }
    ...
    $.fn.epGetTipManager = function ()
    {
        return $.epui.get(this);
    };


    $.epDefaults = $.epDefaults || {};
    ...
    //在指定Dom Element右侧显示气泡,通过$.fn.epTip调用
    $.epDefaults.ElementTip = {
        distanceX: 1,
        distanceY: -3,
        auto: null,
        removeTitle: true
    ...
    $.epui.controls.Tip = function (options)
    {
        $.epui.controls.Tip.base.constructor.call(this, null, options);
    };
    $.epui.controls.Tip.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return 'Tip';
        },
        ...
        _render: function ()
        {
            ...
            g.tip = tip;
            g.tip.attr("id", g.id);
			var tipCon=$('<div class="ep-tip-con"></div>');
        ...
        remove: function ()
        {
            ...
            if (this.options.target)
            {
                ...
                this.options.target.epuitipid = null;
            }
            this.tip.remove();
        }
    });
...
(function ($)
{
    ...
    {
        return $.epui.run.call(null, "epMenu", arguments);
    };
    ...
    $.epui.controls.Menu = function (options)
    {
        $.epui.controls.Menu.base.constructor.call(this, null, options);
    };
    $.epui.controls.Menu.epExtend($.epui.core.UIComponent, {
        __getType: function ()
        {
            return 'Menu';
        },
        ...
        addItem: function (item, menu)
        {
            ...
            ditem.attr("epuimenutemid", ++g.menuItemCount);
            ...
            if (item.children)
            {
                ...
                var newmenu = g.createMenu(ditem.attr("epuimenutemid"));
                g.menus[ditem.attr("epuimenutemid")] = newmenu;
            ...
            ditem.hover(function ()
            {
                ...
                if (item.children)
                {
                    var epuimenutemid = $(this).attr("epuimenutemid");
                    if (!epuimenutemid) return;
                    if (g.menus[epuimenutemid])
                    {
                        g.show({ top: itemtop, left: $(this).offset().left + $(this).width() - 5 }, g.menus[epuimenutemid]);
        ...
        hideAllSubMenu: function (menu)
        {
            ...
            $("> .l-menu-item", menu.items).each(function ()
            {
                if ($("> .l-menu-item-arrow", this).length > 0)
                {
                    var epuimenutemid = $(this).attr("epuimenutemid");
                    if (!epuimenutemid) return;
                    g.menus[epuimenutemid] && g.hide(g.menus[epuimenutemid]);
                }
            });
        ...
        createMenu: function (parentMenuItemID)
        {
            ...
            parentMenuItemID && menu.attr("epuiparentmenuitemid", parentMenuItemID);
    ...
    //旧写法保留
    $.epui.controls.Menu.prototype.setEnable = $.epui.controls.Menu.prototype.setEnabled;
    $.epui.controls.Menu.prototype.setDisable = $.epui.controls.Menu.prototype.setDisabled;
})(jQuery);